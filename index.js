...
function findIndexByProperty(arr, name) {
  if (!Array.isArray(arr)) {
    ...
    const foundIndex = arr.findIndex(
      ({ name: userName }, index, arr) => name === userName
    );
    return foundIndex;
  }
...
function getPositiveArrayValues(arr) {
  if (!Array.isArray) {
    ...
    const positiveArrayValues = Array.from(iterator).filter(
      (value) => value > 0
    );
...
function removeAndDouble(arr, startIdx) {
  if (!Array.isArray) {
    ...
    const smallArray = arr.splice(startIdx);
...
function sumAndPrint(arr) {
  if (!Array.isArray) {
    ...
    const sum = arr.reduce((acc, currentValue, currentIndex, array) => {
...
function getLastElementsSumAndPrint(arr, n) {
  if (!Array.isArray) {
    ...
    const smallArr = arr.slice(n + 1);

    const sum = smallArr.reduceRight(
...
function wordLengths(arr) {
  if (!Array.isArray) {
    ...
    const lengthsArray = arr.map((el, index, array) => {
      return el.length;
    });
    return lengthsArray;
  }
...
function insertElementAt(arr, index, element) {
  if (!Array.isArray) {
    ...
    const arr1 = arr.splice(index, 0, element);
    return arr;
  }
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  if (!Array.isArray && typeof n === "number") {
...
function findLongestWord(arr) {
  if (!Array.isArray) {
    ...
    const longestWord = arr.reduce((longestWord, currentValue) => {
      return currentValue.length > longestWord.length
        ? currentValue
        : longestWord;
    }, "");
    return longestWord;
...
function calculateTotalPrice(arr) {
  if (!Array.isArray) {
    ...
    const totalPrice = arr.reduce((num, item) => {
